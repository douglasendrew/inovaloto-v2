...
$dispatcher = simpleDispatcher(function (RouteCollector $r) {
    $r->post('/master/login', [App\Controllers\MasterAuthController::class, 'login']);
    $r->get('/master/bancas', [App\Controllers\BancaController::class, 'index']);
    $r->post('/master/bancas', [App\Controllers\BancaController::class, 'store']);
    $r->put('/master/bancas/{uuid}', [App\Controllers\BancaController::class, 'update']);
    $r->post('/master/bancas/{uuid}/toggle', [App\Controllers\BancaController::class, 'toggleStatus']);
});

$host = $_ENV['HTTP_HOST'] ?? '0.0.0.0';
$port = (int)($_ENV['HTTP_PORT'] ?? 8001); // master on port 8001 by default
...
$server->on('start', function ($server) use ($host, $port) {
    echo "Swoole HTTP Master Server started at http://{$host}:{$port}\n";
});
